...
function traduceDatos (codigoLenguaje, mensaje){
    ...
    axios.post(direccionTraduceIdioma, bodyTraduccion, {
        ...
        .then(respuesta => {
        FromStringToArray( respuesta.data[0].translations[0].text, codigoLenguaje );
    })
    .catch(error => console.log (error.data));
}

function FromStringToArray(emergencia, codigoLenguaje) {
    var arreglo=[], arreglo2=[];
    for (let index = 0; index <emergencia.length; index++) {
        if (emergencia[index]!=" ") {
            arreglo.push(emergencia[index]);
            if (index==emergencia.length-1) {
                arreglo2.push(arreglo.join(""));
            }
        }else{
            arreglo2.push(arreglo.join(""));
            arreglo = []
    ...
    detectarSituacion(arreglo2, codigoLenguaje);
}
...
function PreguntarDesicion(direccionTraduceIdioma2) {
    var decision = prompt ("Si desea añadir consultar por otra emergencia presione 1: ");
    if (decision == 1){
...
function opciones(direccionTraduceIdioma2, situaciones_detectadas) {
    ...
    switch (situaciones_detectadas[IndexOpcions-1]) {
        case "incendio":
            ...
            break;
        case "tiroteo":
                var help = [{"text" : biblioteca.find(1)}];
                Traducir(direccionTraduceIdioma2, help);
            break;
        case "herida":
